...
There are a large number of missing values in the revenue_omu column and the column is treated as <chr> data type instead of numeric. Before we continue, we will change revenue_omu datatype to numeric.
...
::: callout-note
Observing the nodes datatable above, we will notice that some of the node ids are not unique, some may have more than 1 country, offer more than 1 product services and/or more than 1 revenue reflected.
:::
...
### 4.2.2 Checking for duplicate nodes and removing them
...
### 4.2.4 Distribution of the product_services
...
Looking at the above pie chart, we can see that companies that offer fishing-related products only take up a small percentage, around 4% and the rest are either non-fishing related or unknown. 
